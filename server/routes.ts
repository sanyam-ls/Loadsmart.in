...
export async function registerRoutes(
  ...
  app.post("/api/admin/price", requireAuth, async (req, res) => {
    try {
      ...
      // ATOMIC: Create invoice FIRST before updating load status
      // This ensures if invoice creation fails, load status remains unchanged
      let generatedInvoice = null;
      const existingInvoice = await storage.getInvoiceByLoad(load_id);
      if (!existingInvoice) {
        const invoiceNumber = await storage.generateInvoiceNumber();
        const subtotal = parseFloat(final_price);
        const taxPercent = 18;
        const taxAmount = subtotal * (taxPercent / 100);
        const totalAmount = subtotal + taxAmount;
        
        generatedInvoice = await storage.createInvoice({
          invoiceNumber,
          loadId: load_id,
          ...
          notes: `Auto-generated invoice for load ${load.pickupCity} to ${load.dropoffCity}`,
          lineItems: [
            {
              description: `Freight Transport: ${load.pickupCity} to ${load.dropoffCity}`,
              quantity: 1,
              rate: subtotal,
              amount: subtotal
            }
          ...
          platformMargin: "0",
          estimatedCarrierPayout: "0",
          status: "draft",
          idempotencyKey: `auto-invoice-${load_id}-${Date.now()}`,
        });

        // Create audit log for auto-generated invoice
        await storage.createInvoiceHistory({
          invoiceId: generatedInvoice.id,
          userId: user.id,
          action: "create",
          payload: { autoGenerated: true, source: "price_and_post" },
        });
      } else {
        // Validate existing invoice is linked correctly
        if (existingInvoice.loadId !== load_id) {
          return res.status(400).json({ error: "Invoice load mismatch" });
        }
        // Allow all non-terminal invoice states - terminal states are paid/cancelled/refunded
        const terminalStates = ['paid', 'cancelled', 'refunded'];
        if (terminalStates.includes(existingInvoice.status || '')) {
          return res.status(400).json({ 
            error: `Cannot reuse invoice in ${existingInvoice.status} state. Please create a new invoice.` 
          });
        }
        generatedInvoice = existingInvoice;
      }

      // Update load with admin pricing AFTER invoice is created successfully
